// import React from 'react';
import React, { useState, useEffect } from 'react';
import '../CSS files/store.css';
import ItemsAPI from '../ApI/ItemsAPI';
import { NavLink } from 'react-router-dom';

const catogary = [...new Set(ItemsAPI.map((items) => {
    return items.item_catagory
}))]

const Store = () => {
    const [Slide_bar, setSlide_bar] = useState('off');
    const [Api, setAPI] = useState(ItemsAPI);
    const [filter_items, setfilter_items] = useState(ItemsAPI)
    const [Navbar, setNavbar] = useState('navbar-start')
    useEffect(() => {
        localStorage.setItem('periouspage', (localStorage.getItem('page')));
        localStorage.setItem('page', (window.location.pathname));
        if (localStorage.getItem('periouspage').length > 1) {
            setNavbar('')
        }
    }, [])
    
    const slide_bar_funtion = () => {
        if (Slide_bar === 'on') {
            setSlide_bar('off')
        } else {
            setSlide_bar('on')
        }
    }
    const detail = (e) => {
        if (e.target.innerText === 'Less Detail') {
            e.target.innerText = 'More Detail'
            e.target.parentElement.nextSibling.style.display = 'none'
            e.target.parentElement.nextSibling.nextSibling.style.display = 'none'
        } else {
            e.target.innerText = 'Less Detail'
            e.target.parentElement.nextSibling.style.display = 'inherit'
            e.target.parentElement.nextSibling.nextSibling.style.display = 'inherit'
        }
    }
    const filter = (e) => {
        if (e.target.value === 'ALL') {
            setAPI(ItemsAPI)
            setfilter_items(ItemsAPI)
        } else {
            const filter_Api = ItemsAPI.filter((filter) => {
                return filter.item_catagory === e.target.value
            })
            setAPI(filter_Api)
            setfilter_items(filter_Api)
        }

    }
    const buy = (e) => {
        console.log(e.target.id)
    }
    const send_comment = (e) => {
        if (e.target.previousElementSibling.value === '') {
            alert('please fill the comment')
        } else if (e.target.previousElementSibling.value > 100) {
            alert('Comment will be less than 100 words')
        } else {
            console.log(e.target.id)
            console.log(e.target.previousElementSibling.value.toString());
            e.target.previousElementSibling.value = '';
        }
    }
    const search = (e) => {
        if (e.target.value.length === 0) {
            setAPI(filter_items)
        } else {
            const searchfilter = Api.filter((itemss) => {
                return itemss.item_name.toLowerCase().includes(e.target.value.toLowerCase())
            })
            setAPI(searchfilter)
        }
    }
    const navbar_animation = () => {
        setNavbar('')
    }

    return (
        <div className='store'>
            <div className='nav-bar' id={Navbar} onAnimationEnd={navbar_animation}>
                <span>
                    <i className='fas fa-bars' value='hello' onClick={slide_bar_funtion}></i>
                    <h1>HEYJOYE</h1>
                    <i className='fas fa-search' value='hello' onClick={slide_bar_funtion}></i>
                </span>
                <div className='slide-bar' id={Slide_bar}>
                    <div className='nav-button'>
                        <NavLink className='nav-buttons' to='/'>Home</NavLink>
                        <NavLink className='nav-buttons' to='/support'>F&Q</NavLink>
                        <NavLink className='nav-buttons' to='/login'>Login</NavLink>
                        <NavLink className='nav-buttons' to='/product'>Product</NavLink>
                    </div>
                    <div className='search-tab'>
                        <div>
                            <input type='search' onChange={search} name='search-input' id='search' spellCheck='false' placeholder='Search Items' autoComplete="off" />
                            <button type='submit' id='search-button'><i className='fas fa-search'></i></button>
                        </div>
                        <select onChange={filter}>
                            <option value={'ALL'}>ALL</option>
                            {catogary.map((catogary) => {
                                return (
                                    <option value={catogary} key={catogary}>{catogary}</option>
                                )
                            })}
                        </select>
                    </div>
                </div>
            </div>
            <section id='item-section'>
                {Api.map((item) => {
                    return (
                        <div className='goods' id='redmibook-pro' key={item.id}>
                            <div className='card'>
                                <figure className='image'>
                                    <img src={item.item_picture} alt='' />
                                </figure>
                                <h1>{item.item_name}</h1>
                                <div className='prices'>
                                    <span className='normal-currency-prices'>{item.item_price}
                                        <i className='fas fa-dollar-sign'></i>
                                    </span>
                                    <span className='bitcoin-prices'>{item.item_bitcoin_price}
                                        <i className='fab fa-bitcoin'></i>
                                    </span>
                                </div>
                                <button className='buy-now' onClick={buy} id={item.id}>
                                    <i className='fas fa-shopping-cart'></i>BUY NOW
                                </button>
                                <button className='more-detail' onClick={detail}>More Detail</button>
                            </div>
                            <article className='description'>
                                {item.item_description}
                            </article>
                            <div className='comment'>
                                <div action='#' className='comment-writter' name='redmibook-pro-comment'>
                                    <input type='text' name='redmibook pro' spellCheck='false' className='comment-text'
                                        id='comment-text' placeholder='Add Your Opinion' />
                                    <button onClick={send_comment} className='comment-buutton' id={item.id}>Add</button>
                                </div>
                                <span className='real-comment'>
                                    <img className='user-image' src='data:image/jpeg;base64,' alt='' />
                                    <aside className='user-opinion'>I Like THis Laptop his prosesor Is Powerfull</aside>
                                </span>
                                <span className='real-comment'>
                                    <img className='user-image' src='data:image/jpeg;base64,' alt='' />
                                    <aside className='user-opinion'>This design is amazing</aside>
                                </span>
                            </div>
                        </div>
                    )
                })}
            </section >
        </div>
    )
}

export default Store